...
import type MpqFile from "../../mpq/file";
...
import type { TriggerData } from "../wtg/triggerdata";
...
export default class W3XParser {
  private readonly archive: MPQArchive;
  private readonly readonly: boolean;
  private readonly bs: BinaryStream;
  private readonly map: any;
  private readonly imports: War3MapImp = new War3MapImp();

  constructor(buffer: Uint8Array, readonly: boolean = true) {
    const bs = this.bs = new BinaryStream(buffer);
    if (bs.readBinary(4) !== 'HM3W') {
      throw new Error('Not a valid W3X map.');
    }

    this.map = {};
    this.map.u1 = bs.readUint32();
    this.map.name = bs.readNull();
    this.map.flags = bs.readUint32();
    this.map.maxPlayers = bs.readUint32();
  ...
  save(): Uint8Array {
    ...
    if (!information || information.getBuildVersion() < 131) {
      ...
      stream.writeUint32(this.map.u1);
      stream.writeNull(this.map.name);
      stream.writeUint32(this.map.flags);
      stream.writeUint32(this.map.maxPlayers);
  ...
  setImportsFile(): boolean {
    ...
    if (this.imports.entries.size > 0) {
      return this.set('war3map.imp', this.imports.save().buffer as ArrayBuffer);
    }
