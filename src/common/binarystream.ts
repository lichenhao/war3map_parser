...
import { float32ToUint8, float64ToUint8, int16ToUint8, int32ToUint8, int8ToUint8, uint16ToUint8, uint32ToUint8, uint8ToFloat32, uint8ToFloat64, uint8ToInt16, uint8ToInt32, uint8ToInt8, uint8ToUint16, uint8ToUint32 } from "./typecast";
...
export default class BinaryStream {
  ...
  constructor(buffer: ArrayBuffer | Uint8Array) {
    ...
    this.remaining = this.byteLength;
  }
  get uint8array () {
    return this.buffer;
  }
  substream(byteLength: number): BinaryStream {
    ...
    return new BinaryStream(this.buffer.subarray(index, index + byteLength));
  }
  ...
  writeInt8(value: number): void {
    this.buffer[this.index] = int8ToUint8(value);
    this.index += 1;
  }
